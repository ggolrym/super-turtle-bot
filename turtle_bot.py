...
# 🌟 락 회피 1단계: 로컬 파일(portfolio.json)을 읽는 대신 구글 시트에서 장부 가져오기 (타임아웃 30초)
portfolio = {}
...
dashboard_list = [] # 🌟 대시보드로 보낼 상세 데이터 리스트
...
if kis_token: 
    for ticker, name in all_stocks.items():
        try:
            ...
            # 🌟 매매가 끝난 후, 최종 보유 중인 종목만 대시보드 리스트에 추가
            if ticker in portfolio:
                pos = portfolio[ticker]
...
# 🌟 락 회피 2단계: 파일에 쓰는 대신 구글 시트로 장부(portfolio)와 대시보드를 통합 전송 (타임아웃 30초)
if SHEET_WEBHOOK_URL:
    sheet_data = {
        ...
        "portfolio": portfolio # 🌟 핵심: 깃허브 로컬 파일 변조 없이 장부를 구글 DB에 저장
    }
