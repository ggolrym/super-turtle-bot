# ==========================================
# 🐢 AI 터틀 봇 v10.1-FULL-TEST (전 종목 스캔 + 극단적 매수 완화판)
# ⚠️ 주의: 1,451개 중 10개를 무차별 매수합니다. 테스트 후 즉시 원상복구!
# ==========================================
import os
...
print(f"\n🤖 총 {len(all_stocks)}개 전 종목 정밀 스캔 시작 (테스트 강제 매수판)!\n")

# ==========================================
# 🌟 5. [핵심] 전 종목 스캔 + 매매 조건 극단적 완화
# ==========================================
if kis_token: 
    for ticker, name in all_stocks.items():
        try:
            ...
            # 💡 [조건 완화] 200일 치 데이터 -> 10일 치만 있어도 통과
            ...
            # 📂 B. 신규 진입 로직 (조건 극단적 완화)
            elif ticker not in portfolio:
                # 💡 [조건 완전 삭제] 유동성, 120일선, 200일선 방어막 모두 삭제
                # if turnover_krw < MIN_TURNOVER_KRW: continue
                # if not is_above_120: continue 
                
                # 💡 [조건 완화] 20일 고점 돌파 -> 어제 고가만 살짝 넘어도 무조건 매수 신호!
                recent_1_high = float(stock_data['High'].iloc[-2:-1].max())
                
                if current_price >= recent_1_high:
                    if current_positions + 1 > MAX_POSITIONS: 
                        skipped_signals.append(f"- [{name}] 10종목 한도 초과 보류")
                    elif current_sector_positions[sector] + 1 > MAX_SECTOR_POSITIONS: 
                        skipped_signals.append(f"- [{name}] {sector} 섹터 초과")
                    else:
                        # ✨ 여기서 실제 한국투자증권 API로 매수 신호가 쏘아집니다!
                        order_res = execute_order(ticker, unit_size, side="BUY", price=current_price)
                        buy_signals.append(f"- [{name}] ✨ [강제 매수 테스트] 신규 진입 ({unit_size}주) ➞ {order_res['msg']}")
        ...
        time.sleep(0.15) # 야후 차단 방지 (1,400개 스캔을 위해 필수)
...
if not kis_token:
    ...
    if buy_signals:
        final_content = f"🤖 **터틀 펀드 [전 종목 API 체결 테스트]** 🤖\n\n**[강제 매수 성공]**\n{buy_text}\n\n**[매수 보류 (한도초과)]**\n{skip_text}\n\n⚠️ **주의:** 실제 체결이 확인되었습니다! 봇이 쓸데없는 잡주를 10개나 샀습니다. **즉시 v10.1 원본 코드로 복구하십시오!**"
    else:
        final_content = f"🤖 **터틀 펀드 [전 종목 API 체결 테스트]** 🤖\n1,400개 종목을 뒤졌으나 어제보다 오른 종목이 단 하나도 없습니다."
...
if SHEET_WEBHOOK_URL:
    ...
    summary_msg = f"[전 종목 테스트] 강제 매수 {buy_count}건 발생" if buy_count > 0 else "특이사항 없음"
