# ==========================================
# 🐢 AI 멀티 에셋 터틀 봇 v10.2 (시공간 지능 탑재 리얼타임 스캔판)
# ==========================================
import os
...
if SHEET_WEBHOOK_URL:
    ...
    except Exception as e: print(f"장부 불러오기 실패: {e}")
...
if target_market in ['KR', 'ALL']:
    ...
    except Exception as e: print(f"⚠️ 코스피 로드 에러: {e}")

# 🇺🇸 미국장 타임이면 S&P500 수집
if target_market in ['US', 'ALL']:
    all_stocks.update({
        'SPLG': 'SPDR S&P 500', 'GLDM': 'SPDR Gold', 
        'DBC': 'Invesco Commodity', 'SH': 'Short S&P500', 'PSQ': 'Short QQQ' 
    })
    try:
    ...
    except Exception as e: print(f"⚠️ S&P500 로드 에러: {e}")

# 🌟 보유 종목은 스캔 대상에 무조건 추가 (대시보드 실시간 갱신용)
...
if kis_token: 
    for ticker, name in all_stocks.items():
        try:
            ...
            # 🌟 [리얼타임 분기] 열리지 않은 반대편 시장의 주식은 대시보드 갱신만 하고 매매 건너뜀
            if target_market != 'ALL' and ticker_market != target_market:
                if ticker in portfolio:
                    pos = portfolio[ticker]
                    dashboard_list.append({
                        "name": pos['name'], "units": pos['units'],
                        "current_price": round(current_price, 2), "buy_price": round(pos['last_buy_price'], 2),
                        "stop_loss": round(pos['stop_loss'], 2), "trailing_stop": round(low_10, 2)
                    })
                continue # 다음 종목으로 패스!
            
            # 여기서부터는 타겟 시장의 종목만 실행됨 (매매 로직)
...
if not kis_token:
    ...
    if buy_signals or sell_signals or skipped_signals:
        ...
        final_content = f"🤖 **터틀 펀드 v10.2 ({market_title} 마감)** 🤖\n{response_text}"
    else:
        final_content = f"🤖 **터틀 펀드 v10.2 ({market_title} 관망)** 🤖\n감시 종목 {len(all_stocks)}개 / 보유 종목 {current_positions}/{MAX_POSITIONS} 개."
