...
def execute_order(ticker, qty, side="BUY", price=0.0):
    if not kis_token or qty <= 0: return {"success": False, "msg": "불가"}
    time.sleep(0.5) 
    ...
    try:
        ...
        return {"success": res.json().get("rt_cd") == "0", "msg": res.json().get('msg1', '')}
    except Exception: return {"success": False, "msg": "통신에러"}
...
US_FEE, US_SEC_FEE, US_CGT_RATE, US_CGT_DEDUCTION = 0.0025, 0.0000206, 0.22, 2500000       

buy_signals, sell_signals, dashboard_list = [], [], [] 
portfolio, cooldown_tracker = {}, {}
...
def check_macro_regime(index_ticker):
    try:
        ...
        ma20, ma50 = float(df['Close'].rolling(20).mean().iloc[-1]), float(df['Close'].rolling(50).mean().iloc[-1])
            ...
            if res.status_code == 200:
                ...
                if raw_text.startswith('{') and raw_text.endswith('}'):
                    ...
                    if isinstance(data, dict): 
                        ...
                        else: bot_cash = INITIAL_CAPITAL - sum(p.get('buy_price', 0) * p.get('units', 0) * (1 if str(p.get('market', '')).startswith('KR') else EXCHANGE_RATE) for p in portfolio.values() if isinstance(p, dict))
                break
            time.sleep(5)
        except: time.sleep(5)
...
for t, expire_date in cooldown_tracker.items():
    try:
        if datetime.strptime(expire_date, '%Y-%m-%d').date() > kr_time.date(): active_cooldowns[t] = expire_date
    except: pass
cooldown_tracker = active_cooldowns

# 💡 [버그 패치] 실전 해외주식 잔고조회 TR_ID 수정 연동
def sync_portfolio_with_kis_balance(current_portfolio):
    ...
    try: 
        ...
        if res.status_code == 200 and res.json().get('rt_cd') == '0':
            ...
            for item in res.json().get('output1', []):
                if int(item.get('hldg_qty', 0)) > 0: kr_tickers[item.get('pdno')] = {'qty': int(item.get('hldg_qty')), 'avg_price': float(item.get('pchs_avg_pric', 0))}
    except: pass

    try: 
        # 💡 모의투자는 VTTS3012R, 실전투자는 JTTT3012R 사용
        headers["tr_id"] = "VTTS3012R" if "openapivts" in KIS_URL else "JTTT3012R"
        ...
        if res.status_code == 200 and res.json().get('rt_cd') == '0':
            ...
            for item in res.json().get('output1', []):
                ...
                if qty > 0:
                    ...
                    if sym and sym != 'BAX': us_tickers[sym] = {'qty': qty, 'avg_price': float(item.get('pchs_avg_pric', 0))}
    except: pass
    ...
    for t, p in current_portfolio.items():
        ...
        clean_t, is_kr = t.split('.')[0], (t.endswith('.KS') or t.endswith('.KQ'))
        
        if is_kr and kr_api_success and (clean_t not in kr_tickers and t not in kr_tickers):
            sell_signals.append(f"🔪 장중 손절 감지: {p.get('name')}")
            bot_cash += (p['units'] * p['buy_price'] * (1 - FIXED_STOP_LOSS_KR) * (1 - KR_FEE - (KR_KOSPI_TAX if t.endswith('.KS') else KR_KOSDAQ_TAX)))
        ...
        if is_kr:
            if not kr_api_success: synced[t] = p; continue
            ...
            if not us_api_success: synced[t] = p; continue
...
for t in portfolio.keys():
    if t not in all_stocks: all_stocks[t] = ('KR_KOSPI' if t.endswith('.KS') else 'KR_KOSDAQ' if t.endswith('.KQ') else 'US', portfolio[t].get('name', t), t)
...
for i, (ticker, (market, name, code)) in enumerate(shuffled_stocks):
    ...
    try:
        ...
        if market.startswith('KR'):
            for _ in range(3):
                try:
                    ...
                    if not temp_data.empty and len(temp_data) > 120: stock_data = temp_data; break
                except: pass
                time.sleep(random.uniform(0.1, 0.3)) 
        else:
            for _ in range(3):
                try:
                    temp_data = yf.Ticker(ticker).history(period='1y')
                    if not temp_data.empty and len(temp_data) > 120: stock_data = temp_data; break
                except: pass
        ...
        gain, loss = (delta.where(delta > 0, 0)).rolling(2).mean(), (-delta.where(delta < 0, 0)).rolling(2).mean()
...
current_portfolio_value = sum(p['units'] * prices_cache.get(t, p.get('buy_price', 0)) * (1 if str(p.get('market', '')).startswith('KR') else EXCHANGE_RATE) for t, p in portfolio.items())
...
if kis_token:
    if kr_time.month == 12 and kr_time.day >= 24:
        ...
        if rem_deduct > 0:
            for ticker in list(portfolio.keys()):
                if portfolio[ticker].get('market') != 'US' or ticker not in data_store: continue
                df, pos = data_store[ticker], portfolio[ticker]
                curr_price, buy_price = float(df['Close'].iloc[-1]), float(pos.get('buy_price', 0))
                if buy_price <= 0: continue
                profit_per_share = (curr_price * (1 - US_FEE - US_SEC_FEE) * EXCHANGE_RATE) - (buy_price * (1 + US_FEE) * EXCHANGE_RATE)
                if profit_per_share > 0:
                    ...
                    if shares_to_sell > 0:
                        ...
                        if res['success']:
                            ...
                            bot_cash += (shares_to_sell * curr_price * EXCHANGE_RATE) * (1 - US_FEE - US_SEC_FEE)
                            yearly_us_profit += realized_profit
                            rem_deduct -= realized_profit
                            sell_signals.append(f"🌟 연말 절세 실현: {pos['name']} | {shares_to_sell}주")
    ...
    for ticker in list(portfolio.keys()):
        ...
        curr_price, is_kr = float(df['Close'].iloc[-1]), market.startswith('KR')
        buy_price = float(pos.get('buy_price', 0)) if float(pos.get('buy_price', 0)) > 0 else curr_price
        pos['hold_days'] += 1
        sl_price = buy_price * (1.0 - (FIXED_STOP_LOSS_KR if is_kr else FIXED_STOP_LOSS_US))
        tp_ma = float(df['MA10'].iloc[-1]) if is_kr else (float(df['MA10'].iloc[-1]) if (curr_price / buy_price - 1.0) >= 0.10 else float(df['MA20'].iloc[-1]))
        
        sell_reason = None
        if curr_price <= sl_price: sell_reason = "하드스탑"; cooldown_tracker[ticker] = (kr_time + timedelta(days=5)).strftime('%Y-%m-%d')
        elif curr_price > buy_price and curr_price < tp_ma: sell_reason = "스마트 익절" 
        elif pos['hold_days'] >= MAX_HOLD_DAYS and curr_price <= buy_price: sell_reason = "타임스탑 탈출"
                
        if sell_reason:
            if execute_order(ticker, pos['units'], side="SELL", price=curr_price)['success']:
                sell_signals.append(f"🔴 청산: {name} | 사유: {sell_reason}")
                bot_cash += (pos['units'] * curr_price * (1 if is_kr else EXCHANGE_RATE)) * (1 - ((KR_FEE + (KR_KOSPI_TAX if market == 'KR_KOSPI' else KR_KOSDAQ_TAX)) if is_kr else (US_FEE + US_SEC_FEE)))
                if not is_kr: yearly_us_profit += pos['units'] * ((curr_price * (1 - US_FEE - US_SEC_FEE) * EXCHANGE_RATE) - (buy_price * (1 + US_FEE) * EXCHANGE_RATE))
    ...
    for ticker, df in data_store.items():
        ...
        if market == 'US':
            if curr_price > ma_120 and curr_price >= float(df['Recent20High'].iloc[-1]) and ((float(df['Volume'].iloc[-1]) >= float(df['VolMA20'].iloc[-1]) * 1.2) if float(df['VolMA20'].iloc[-1]) > 0 else True):
    ...
    for cand in (us_candidates + kr_candidates):
        ...
        if len(portfolio) >= MAX_POSITIONS or (is_kr and current_kr_positions >= MAX_KR_POSITIONS) or (not is_kr and current_us_positions >= MAX_US_POSITIONS): continue
        if execute_order(cand['ticker'], cand['units'], side="BUY", price=cand['price'])['success']:
...
for ticker, pos in portfolio.items():
    ...
    dashboard_list.append({
        "name": pos.get('name', ticker), "units": pos.get('units', 0), 
        "buy_price": round(bp, 2), "current_price": round(prices_cache.get(ticker, bp), 2), 
        "invested": round(bp * pos.get('units', 0), 2), 
        "return_pct": round((((prices_cache.get(ticker, bp) / bp) - 1) * 100) if bp > 0 else 0, 2), 
        "stop_loss": round(bp * (1 - (FIXED_STOP_LOSS_KR if pos.get('market', '').startswith('KR') else FIXED_STOP_LOSS_US)), 2),
        "trend_exit": round(float(data_store[ticker]['MA10'].iloc[-1]) if ticker in data_store else 0, 2)
    })

msg_lines = [f"🤖 **V36.6 Safe Defender Live (자산: {int(total_bot_equity):,}원)** 🤖\n"]
if buy_signals or sell_signals:
    if buy_signals: msg_lines.append("**[🚀 신규 매수]**"); msg_lines.extend(buy_signals)
    if sell_signals: msg_lines.append("\n**[💰 청산/익절]**"); msg_lines.extend(sell_signals)
else: msg_lines.append("💤 발생한 신규 신호 없음.\n")

msg_lines.append(f"\n**[💼 보유 포트폴리오]**")
if not portfolio: msg_lines.append("현금 대기 중")
else:
    for ticker, pos in portfolio.items():
        cp, bp = prices_cache.get(ticker, pos.get('buy_price', 0)), pos.get('buy_price', 0)
        ret = ((cp / bp) - 1) * 100 if bp > 0 else 0
        msg_lines.append(f"🔸 {pos.get('name')}: {fmt_price(bp, pos.get('market', '').startswith('KR'))} ➔ {fmt_price(cp, pos.get('market', '').startswith('KR'))} ({ret:+.2f}%)")
...
if SHEET_WEBHOOK_URL:
    try:
        requests.post(SHEET_WEBHOOK_URL, json={
            ...
            "message": f"자산 {int(total_bot_equity):,}원", 
