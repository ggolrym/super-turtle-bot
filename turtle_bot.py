...
# (4슬롯 분산 / 코스닥 통합 방어막 패치 🌟 / 스마트 익절 / IP 밴 원천차단 / 연말 절세 탑재)
# ==========================================
import os
...
# 정밀 수수료 및 세금 설정 (백테스터 연동)
KR_FEE = 0.00015
KR_KOSPI_TAX = 0.0005
KR_KOSDAQ_TAX = 0.0018
US_FEE = 0.0025        
...
                        else:
                            yearly_us_profit = 0 # 해가 바뀌면 한도 초기화
                            
                        if 'bot_cash' in data: 
...
            print(f"⚠️ {t} 잔고 없음 (장중 서버 자동손절 감지) ➔ 쿨다운 5일 적용")
            sell_signals.append(f"🔪 서버 장중 손절 감지: {p.get('name')} (현금 회수 완료)")
            # 이 경우 수익/손실액을 정확히 알 수 없으므로 우선 현금 복원만 진행
...
    # 12월 24일 이후, 비과세 한도(250만 원)가 남아있다면 수익 종목 분할/전량 자동 매도
    # -----------------------------------
...
        "yearly_us_profit": yearly_us_profit, # 💡 연도별 수익금 구글 시트에 백업
        "current_year": current_year          # 💡 연도 저장
    }
    try: 
